...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    fprintf(fp, "# This data was took by the \"getwaves\" program with the GPIB-USB library.\n");
    fprintf(fp, "# Time: %s\n", timestr);
    ...
    storeRootFile(wpre1, wpre2, wave, volt);
...
static void storeRootFile(const Wfmpre *wpre1, const Wfmpre *wpre2,
  ...
  for (int i = 0; i < 2; i++) {
    ...
    /*    if (i == 0)   f->WriteTObject(wpre1);
	  else   f->WriteTObject(wpre2);*/
    f->WriteTObject(h[i]);
